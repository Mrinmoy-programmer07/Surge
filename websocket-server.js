...
function handleMessage(ws, message) {
  ...
  switch (type) {
    case 'JOIN_QUEUE':
      if (!payload || !payload.playerAddress || !payload.gameType || !payload.stake) {
        ws.send(JSON.stringify({ 
          type: 'ERROR', 
...
function handleJoinQueue(ws, { playerAddress, gameType, stake }) {
  ...
  queue.push(player);
  console.log(`🎮 Player ${playerAddress} joined ${gameType} queue (stake: ${stake})`);
...
function removePlayerFromAllQueues(ws) {
  for (const [queueKey, queue] of matchmakingQueues.entries()) {
    ...
    if (playerIndex !== -1) {
      ...
      console.log(`👋 Player ${player.playerAddress} left ${player.gameType} queue`);
...
function tryMatchPlayers(queueKey) {
  ...
  const matchData = {
    ...
    payload: {
      ...
      gameStartTime: gameStartTime // Add synchronized start time
    }
  };
  
  console.log(`⏰ Game will start at: ${new Date(gameStartTime).toISOString()}`);
...
function handleGameReady(ws, { matchId, playerAddress }) {
  ...
  if (game.player1.isReady && game.player2.isReady) {
    ...
    console.log(`🚀 Game ${matchId} started! Current player: ${game.currentPlayer}`);
  }
}
...
function handleSubmitTurn(game, playerAddress, data) {
  // Switch turns
  game.currentPlayer = game.currentPlayer === game.player1.address 
  ...
  console.log(`🔄 Turn switched to ${game.currentPlayer} in game ${game.matchId}`);
}
...
function handleUpdateScore(game, playerAddress, data) {
  ...
  console.log(`📊 Score updated in game ${game.matchId}: ${game.player1.score} vs ${game.player2.score}`);
}
...
function handleGameOver(game, playerAddress, data) {
  ...
  console.log(`🏁 Game ${game.matchId} finished! Winner: ${data.winner}`);
...
server.on('request', (req, res) => {
  if (req.url === '/health') {
    ...
    res.end(JSON.stringify({
      ...
      totalPlayers: Array.from(matchmakingQueues.values()).reduce((sum, queue) => sum + queue.length, 0),
