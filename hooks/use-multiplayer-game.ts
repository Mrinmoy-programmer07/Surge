...
interface GameMessage {
  type: 'GAME_START' | 'TURN_CHANGE' | 'SCORE_UPDATE' | 'GAME_OVER' | 'ERROR' | 'PONG'
  payload?: any
}
...
export function useMultiplayerGame() {
  ...
  const handleGameMessage = useCallback((message: GameMessage) => {
    ...
    switch (message.type) {
      case 'GAME_START':
        const { matchId, gameType, stake, player1, player2, currentPlayer, gameData } = message.payload
        setGameState(prev => ({
          ...prev,
          matchId,
          gameType,
  ...
  const submitTurn = useCallback((matchId: string, data?: any) => {
    if (!address) return
  ...
  const updateScore = useCallback((matchId: string, score: number, gameData?: any) => {
    if (!address) return
    sendMessage('GAME_ACTION', { 
      matchId, 
      playerAddress: address, 
  ...
  const endGame = useCallback((matchId: string, winner: string, gameData?: any) => {
    if (!address) return
    sendMessage('GAME_ACTION', { 
      matchId, 
      playerAddress: address, 
